"""
NoLatentModel: World model with four MLPs for MPPI planning without latent encoding.
Predicts next state, reward, action, and value from current state and action
Also generates dreams of future states
"""
...
class NoLatentModel(nn.Module):
    ...
    dynamics (obs, action)-> next_obs
    reward (obs, action) -> reward
    policy obs -> action 
    value obs -> V
    """
    ...
    def generate_dreams(self, obs: torch.Tensor, horizon: int) -> torch.Tensor:
        # Dreams are generated by the NLM technique
    ...
    def forward(
        ...
        # Returns predictions and optionally dreams when horizon is provided
